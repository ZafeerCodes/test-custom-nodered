...
def process_frame(frame, model, rois, args, tracker):
    ...
    for det in detections.boxes.data.tolist():
        ...
        if args.isTrackingEnabled and tracker:
            ...
            boxes.append([x1, y1, x2, y2, object_detection_classes[label], prob, tracking_id])
        else:
            x1, y1, x2, y2, prob = det[:5]
            label = int(det[5]) 
            boxes.append([x1, y1, x2, y2, object_detection_classes[label], prob])
 
    if rois:
        return filter_detections_by_rois(boxes, rois)
    else:
        return {"all": boxes}
